...
import { getEmployeeSkillById, updateEmployeeSkillById } from 'apiSdk/employee-skills';
...
function EmployeeSkillEditPage() {
  ...
  const handleSubmit = async (values: EmployeeSkillInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateEmployeeSkillById(id, values);
      mutate(updated);
      resetForm();
