...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getEmployeeSkills() {
    const data = await prisma.employee_skill
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'employee_skill'));
    return res.status(200).json(data);
  }

  async function createEmployeeSkill() {
    await employeeSkillValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.employee_skill.create({
      data: body,
    });
    return res.status(200).json(data);
  }
